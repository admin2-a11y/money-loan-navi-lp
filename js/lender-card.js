(() => {
  ...
  const lockedMobitDiagnosisCopy = Object.freeze({
    ...
    amount: Object.freeze({
      ...
      "30_50": "SMBCモビットは、あなたの希望する30～50万円程度の借入にあわせて検討しやすいサービスです。まとまった資金が必要な場合にも検討しやすく、借入額の融通がききやすいのもポイントです。",
    ...
    priority: Object.freeze({
      ...
      privacy: "SMBCモビットであれば、WEB完結申込なら原則、電話連絡・郵送物なしです。申込状況によってはご案内連絡が必要になる場合があります。",
  ...
  const diagnosisComments = (lender) => {
    ...
    if (lender.key === "mobit") {
      ...
      const firstTimeCopy = {
        ...
        promise: `プロミスはスマホから借入までWEB完結。手続きがカンタンなので、カードローンがはじめてのあなたにおすすめです。`,
        aiful: `アイフルはスマホから借入までWEB完結。手続きがカンタンなので、カードローンがはじめてのあなたにおすすめです。`
      };
    ...
    const focusedAmountCopy = {
      promise: `プロミスは、あなたの希望する30～50万円程度の借入にあわせて検討しやすいサービスです。まとまった資金が必要な場合にも検討しやすく、借入額の融通がききやすい点もポイントです。`,
      aiful: `アイフルは、あなたの希望する30～50万円程度の借入にあわせて検討しやすいサービスです。まとまった資金が必要な場合にも検討しやすく、借入額の融通がききやすい点もポイントです。`
    };
    ...
    const lenderPriorityComments = {
      ...
      promise: {
        ...
        privacy: `プロミスであれば、WEB完結申込は原則、電話での在籍確認や郵送物が少なめ。申込内容によってはご連絡が必要になる場合があります。`,
      ...
      aiful: {
        ...
        privacy: `アイフルは、WEB完結申込なら原則、電話連絡・郵送物が少なめです。申込内容によっては連絡対応が必要になる場合があります。`,
